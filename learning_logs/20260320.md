# 입실 체크 해주세요 !! 🎈
application.properties 설정 살리고 backend server 켜고 해야겠네요.

그리고 heidiSQL을 켜서 `SELECT * FROM users;`를 해보면, ddl-auto=update 설정 때문에 어제 회원 가입한 계정이 그대로 남아있음을 확인할 수 있습니다.

현재 frontend server 켜면 화면은 안나오는데 개발자도구 상 오류는 안나옵니다.
...
`http://localhost:5173/oauth2/callback?token=eyJ....&email=kim1@gmail.com&name=김일&role=ROLE_USER`
라는 전체 url을 return하게 되는데, 그 중에서 `?`뒷 부분에 있는 부분이 key-value형태와 유사하다는 것을 알 수 있습니다. token, email, name, role이라는 key를 갖고, `=` 뒷 부분이 value가 되며, 다음 key로 연결될 때 `&`를 쓴 형태네요. 백엔드에서는 `@PathVariable`을 통해서 endpoint 뒷부분의 id를 동적으로 가져올 수 있었습니다. `http://localhost:8080/api/vehicles/1`과 에서 `/1` 부분이었죠. 근데 search 라고 하는 엔드포인트를 만들어본 적도 있었습니다. repository 내에서 커스텀 메서드를 만들었을 때 였습니다. 그때 저희는 물음표 부분을 확인할 수 있었는데, 이처럼 `?` 뒷부분을 쿼리스트링(query string)이라고 칭합니다.

- 그러면 searchParams가 무슨 역할을 하는가? - 현재 브라우저 주소창에 있는 쿼리 스트링을 읽어오거나 수정할 때 사용함.

- 즉 `const [ searchParams ] = useSearchParams();`라고 쓴 다음에 `searchParams.get('키이름');`을 하면, 예를 들어 key가 token이라면 `http://localhost:5173/oauth2/callback?token=eyJ....&email=kim1@gmail.com&name=김일&role=ROLE_USER`여기서 token=뒷부분부터 & 앞까지의 값을 string으로 가져온다는 의미가 되겠습니다.

- `??` 란 ? (null 병합 연산자)
- JS에서 null 오류를 잡기 위한건데 `??` 왼쪽 값이 null이면 오른쪽 값으로 대입한다는겁니다.
...
- `{replace : true}` : OAuth2 로그인이 완료된 다음에 `/`를 통해 메인페이지로 진입하게 될텐데, 그때 뒤로가기 버튼 누르면 다시 callback page가 나오게 될겁니다. 그러면 불필요한 fetch가 일어날 수 있겠네요. 그거 막아주는 앱니다.
...
1. 오류 생기시는 분들 해결할 것
  - 모르면 질문할 것
2. 자소서 및 이력서 완료하고 반장님께 전송할 것 - 16:00 이후
3. 다 된 분들 자격증 공부

다음주 금요일(27일) 시험 예정

git add .
git commit -m "feat: 20260320 oauth2 back-front finished"
git push
